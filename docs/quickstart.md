...
The example creates one email-send Action Card, approves it with a deterministic renderer, and prints the decision plus resume policy. See [`examples/npm-consumer-basic`](../examples/npm-consumer-basic/README.md) for expected output and the optional TypeScript source.
