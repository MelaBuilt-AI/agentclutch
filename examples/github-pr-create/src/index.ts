...
export async function runGitHubPrCreateExample() {
  ...
  const result = await clutch.onActionProposed({
    ...
    proposedAction: {
      ...
      rawInput: {
        ...
        changedFields: [
          ...
          { field: "changedFiles", after: draft.changedFiles, editable: false }
        ]
      }
    },
    visibleContext: {
    ...
    loopContext: {
      planSummary: "Prepare example changes, open PR, wait for review and checks.",
      whyNow: "The branch is ready for human review before repository state changes.",
      confidence: 0.86
    },
    riskHints: {
